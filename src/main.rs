...
fn main() -> Result<(), reqwest::Error>  {
    ...
    if stations.is_empty(){
        ...
        let mut available_lines: Vec<String> = station_overview.departures.iter().map(|d| d.line.clone()).collect();
            ...
            let line = match fast_args.clone() {
                Some((_, fast_line)) => {
                    if available_lines.contains(&fast_line) {
                        ...
                        println!("{} is not among the available lines {:?}", fast_line, available_lines);
                        process::exit(1);
                    }
                },
                None => {
                    ...
                    available_lines.get(user_line_choice - 1).unwrap().to_string()
                }
            };
...
fn get_station_detail_for_line(station_overview: StationOverview, line: &str) -> Result<StationDetail, reqwest::Error>  {
    let departures_detail_res: Result<Vec<DepartureDetail>, reqwest::Error> = station_overview.departures
        .par_iter()
        .filter(|&d| d.line == line)
        .map(make_departure_detail)
        .collect();
    let departures = departures_detail_res?;
    let mut disruptions: Vec<String> = departures.iter().flat_map(|d| d.information.clone()).collect();
    disruptions.sort();
    disruptions.dedup();
...
fn make_departure_detail(departure_overview: &DepartureOverview) ->  Result<DepartureDetail, reqwest::Error>  {
    ...
    let information: Vec<String> = disruptions_nodes.iter()
        .map(|n| sanitize_text_node(*n))
        .filter(|i| !i.is_empty())
        .collect();

    let query_details = Attr("id", "ivu_trainroute_table").descendant(Name("tr")).descendant(Class("tqTime"));
    let steps_elements_nodes: Vec<Node> = html.find(query_details).collect();
    let steps_elements: Vec<String> = steps_elements_nodes.iter()
        .filter_map(|n| {
            ...
            if txt.starts_with(&departure_overview.time) && txt.len() > 5 {
                ...
                if raw_delay == NO_DELAY { None } else { Some(raw_delay) }
            } else {
                None
            }
        }).collect();
    ...
    let status = delay_minutes.map(|d| format!("delayed {}", d.to_owned())).unwrap_or_else(|| "on time".to_string());
...
fn padding_for_header<F>(departures: &[DepartureDetail], field_selector: F, header_label: &str) -> String
    where F: Fn(&DepartureDetail) -> &str {
    let max_elem_size = departures.iter()
...
fn display_departures(station_detail: StationDetail) -> () {
    if !station_detail.disruptions.is_empty() {
        let pretty_disruption: String = station_detail.disruptions.iter()
        ...
        println!("\n{}\n\n{}\n", "* Service disruption *".red().bold().underline(), pretty_disruption);
    }
    ...
    let direction_header_padding = padding_for_header(&departures, |d| &d.direction, DIRECTION_COLUMN);
    let direction_header_len = DIRECTION_COLUMN.len() + direction_header_padding.len();

    let header = format!("{}{}{}{}{}{}{}{}{}",
                         LINE_COLUMN ,line_header_padding,
                         DEPARTURE_COLUMN, departure_header_padding,
                         STATUS_COLUMN, status_header_padding,
                         DIRECTION_COLUMN, direction_header_padding,
                         PLATFORM_COLUMN);
    ...
    departures.iter().for_each(
        |d| {
            ...
            println!("{}{}{}{}{}{}{}{}{}",
                     ...
                     d.status, after_status_padding,
                     d.direction, after_direction_padding,
...
fn sentence_chunks(s: &str, max_line_len: usize) -> String {
    if s.chars().count() < max_line_len {
        ...
        let (head, tail) =
            if s.is_char_boundary(max_line_len) {
...
fn read_user_choice_range(r: RangeInclusive<usize>) -> usize {
    match read_user_choice_int() {
        ...
        _ => {
            println!("Invalid input - please pick a number between {} and {}", r.start(), r.end());
...
fn get_stations(user_input: &str) -> Result<Vec<StationSearch>, reqwest::Error> {
    ...
    let stations = html.find(Class("select").descendant(Name("a")))
...
fn get_station_overview(station: &StationSearch) -> Result<StationOverview, reqwest::Error> {
    ...
    let name = html.find(Attr("id", "ivu_overview_input").descendant(Name("strong"))).take(1).next().unwrap().text();
    let query = Class("ivu_table").descendant(Name("tbody")).descendant(Name("tr"));
    let elements: Vec<Node> = html.find(query).collect();
    let parsed_blocks: Vec<ParsedInfo> = elements.iter()
        .enumerate()
        .filter_map(|(index, node)| {
            ...
            if blocks.len() == 3 {
                ...
                Some(ParsedInfo{index, time_node:*time_node, line_node:*line_node, direction_node:*direction_node})
            } else {
    ...
    let departures =
        parsed_blocks.iter()
            .map(|pd| {
                ...
                let line_link = pd.line_node.find(Name("a")).into_selection().first().unwrap().attr("href").unwrap();
                ...
                let (line, platform) = if full_line.contains('(') {
                    ...
                    let platform_raw: String = full_line.chars().rev().skip(1).take_while(|c| c != &'.').collect();
